...
#     Yves: header contains/doesn't contain:    true    ${currencySwitcher}[${env}]    ${quickOrderIcon}    ${accountIcon}    ${shoppingListIcon}    ${shoppingCartIcon}
...
#     Yves: go to company menu item:    Users    #Benutzer
...
#     Yves: select the following shipping method on the checkout and go next:    Expedited Two-Day (Air) Delivery
...
#     [Documentation]    editing existing shippment on order in zed
#     Yves: login on Yves with provided credentials:    ${yves_company_user_shared_permission_owner_email}
#     Yves: delete all shopping carts
#     Yves: go to 'Shopping Carts' page through the header
#     Yves: 'Shopping Carts' page is displayed
...
# !!!Volume_Prices-need_clarify_logic
...
# !!!Product_Sets-need_clarify_logic
...
# !!!Product_Relations-need_clarify_logic
...
# !!!Customer_Specific_Prices-need_clarify_logic
...
# !!!Agent_Assist-need_clarify_logic
...
#     Yves: go to second navigation item level:    More    ${configurable_bundle}
...
#     Yves: change quantity using '+' or '-' button № times of the configurable bundle in the shopping cart on:    Presentation bundle    +    1
#     Yves: click on the 'Checkout' button in the shopping cart
#     Yves: billing address same as shipping address:    true
#     Yves: select the following existing address on the checkout as 'shipping' address and go next:    ${yves_company_user_manager_and_buyer_address}
#     Yves: select the following shipping method on the checkout and go next:    Expedited Two-Day (Air) Delivery
...
#     Yves: create a new customer address in profile:    Ms    ${yves_user_first_name} ${random}    ${yves_user_last_name} ${random}   Kirncher Str. ${random}    7    ${random}    Berlin    United States of America
#     Yves: go to user menu item in the left bar:    Addresses
#     Yves: 'Addresses' page is displayed
#     Yves: check that user has address exists/doesn't exist:    true    ${yves_user_first_name} ${random}    ${yves_user_last_name} ${random}    Kirncher Str. ${random}    7    ${random}    Berlin    United States of America
#     Yves: delete user address:    Kirncher Str. ${random}
#     Yves: go to user menu item in the left bar:    Addresses
#     Yves: 'Addresses' page is displayed
#     Yves: check that user has address exists/doesn't exist:    false    ${yves_user_first_name} ${random}    ${yves_user_last_name} ${random}    Kirncher Str. ${random}    7    ${random}    Berlin    United States of America
#     Zed: login on Zed with provided credentials:    ${zed_admin_email}
...
#     ...    || ${yves_user_email} | Ms         | ${yves_user_first_name}${random} | ${yves_user_last_name}${random} | address 1${random} | address 2 ${random} | address 3 ${random} | Berlin${random} | ${random} | United States | 123456789 | Spryker${random} ||
#     Yves: login on Yves with provided credentials:    ${yves_user_email}
#     Yves: go to user menu item in header:    Overview
#     Yves: go to user menu item in the left bar:    Addresses
#     Yves: check that user has address exists/doesn't exist:    true    ${yves_user_first_name}${random}    ${yves_user_last_name}${random}    address 1${random}    address 2 ${random}    ${random}    Berlin${random}    United States of America
...
#     Yves: go to second navigation item level:    All categories    ${stationery}
...
#     Yves: shopping cart contains the following products:    420573    102181
...
#     Yves: discount is applied:    voucher    Voucher Code 5% ${random}    - $0.70
#     Yves: discount is applied:    cart rule    Cart Rule 10% ${random}    - $1.41
#     Yves: go to PDP of the product with sku:    ${bundle_product_abstract_sku}
#     Yves: add product to the shopping cart
#     Yves: go to the shopping cart through the header with name:    discounts+${random}
#     Yves: discount is applied:    cart rule    Cart Rule 10% ${random}    - $105.87
...
#     Yves: discount is applied:    cart rule    Promotional Product 100% ${random}    - $53.72
#     Yves: click on the 'Checkout' button in the shopping cart
#     Yves: billing address same as shipping address:    true
#     Yves: select the following existing address on the checkout as 'shipping' address and go next:    ${yves_company_user_buyer_address}
#     Yves: select the following shipping method on the checkout and go next:    Expedited Two-Day (Air) Delivery
...
#     Yves: get the last placed order ID by current customer
#     Zed: login on Zed with provided credentials:    ${zed_admin_email}
#     Zed: grand total for the order equals:    ${lastPlacedOrder}    $925.84
#     [Teardown]    Run keywords    Zed: login on Zed with provided credentials:    ${zed_admin_email}
...
#     Yves: select the following shipping method for the shipment:    3    DHL    Expedited Two-Day (Air) Delivery
...
#     ...    || 2         | New address      | Mr         | Edit       | Shipment  | ${yves_user_email} | Germany | Hartmanngasse | 9         | Vienna | 0987     | DHL - Expedited Two-Day (Air) Delivery   | 2025-01-25              | 107254 ||
#     Zed: order has the following number of shipments:    ${lastPlacedOrder}    3
#     Zed: shipment data inside xxx shipment should be:
#     ...    || shipment n | delivery method | shipping method | shipping costs | requested delivery date ||
#     ...    || 2          | DHL             | Standard        |  €0.00         | ASAP                    ||
#     Zed: shipment data inside xxx shipment should be:
#     ...    || shipment n | delivery method | shipping method | shipping costs | requested delivery date ||
#     ...    || 3          | DHL             | Expedited Two-Day (Air) Delivery         |  €0.00         | 2025-01-25              ||
